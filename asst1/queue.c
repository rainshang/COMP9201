...
int q_preallocate(struct queue *q, int size)
{
        int result = 0;
...
int q_addtail(struct queue *q, void *ptr)
{
        int nextnext, result;
...
int q_getstart(struct queue *q)
{
        return q->nextread;
}

int q_getend(struct queue *q)
{
        return q->nextwrite;
}

int q_getsize(struct queue *q)
{
        return q->size;
}
